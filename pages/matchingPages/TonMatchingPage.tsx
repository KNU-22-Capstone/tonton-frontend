import React from 'react'

function TonMatchingPage() {
  return (
    <div className='absolute w-full h-full bg-indigo-900 overflow-auto'>
        <div className='absolute left-[45rem] top-[8rem] text-5xl font-extrabold h-auto'>
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-pink-500 to-violet-500">
                톤온톤 톤인톤 매칭 결과
            </span>
            
        </div>
        
        <div className='absolute left-[14rem] top-[15rem] mx-10 z-10'>
            <div className='relative rounded-full border-[0.5rem] border-white bg-slate-400  w-[14rem] h-[14rem] shadow-2xl shadow-black'>    
                <div className='relative top-7'>
                    <div className='absolute right-0 left-0 top-0 bottom-0  w-[8rem] m-auto'>
                        <img
                            className='border-4 border-black'
                            src={"https://image.msscdn.net/images/goods_img/20190201/947088/947088_3_500.jpg?t=20200706103059"}
                        />
                    </div>
                    
                </div>
            </div>
            
        </div>

        <div className='absolute left-[23rem] top-[25rem] w-auto z-0 '>
            <div className='rounded-2xl border-l-[2rem] border-b-[2rem]  border-white border-opacity-40 w-[40rem] h-[68rem] my-10'>
                
                <div className='absolute left-[3rem] top-[3rem] w-auto'>
                    <div className='rounded-3xl border-l-8 border-t-8 border-b-8  border-white w-[74rem] h-[62rem] my-10 z-20 '>
                       
                        <ul className='flex flex-wrap w-[75rem] my-5 z-0'>
                            <li>
                                <div className='relative w-[16rem] h-[17rem] bg-blue-800 shadow-2xl shadow-black mx-5 rounded-3xl hover:opacity-80'>

                                    <img
                                        className='absolute left-4 top-4 border-4 w-[14rem] h-[15rem] border-black'
                                        src={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZR1UEHgChheu3fJHfr7Ddo1ufyy24S2FC_sAkKtlpMgse2RlV-3hLwzvjdFp1B_y0xhk&usqp=CAU"}
                                    />
                                    
                                </div>
                            </li>
                            <li>
                                <div className='relative w-[16rem] h-[17rem] bg-blue-800 shadow-2xl shadow-black mx-5 rounded-3xl hover:opacity-80'>

                                    <img
                                        className='absolute left-4 top-4 border-4 w-[14rem] h-[15rem] border-black'
                                        src={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZR1UEHgChheu3fJHfr7Ddo1ufyy24S2FC_sAkKtlpMgse2RlV-3hLwzvjdFp1B_y0xhk&usqp=CAU"}
                                    />
                                    
                                </div>
                            </li>
                            <li>
                                <div className='relative w-[16rem] h-[17rem] bg-blue-800 shadow-2xl shadow-black mx-5 rounded-3xl hover:opacity-80'>

                                    <img
                                        className='absolute left-4 top-4 border-4 w-[14rem] h-[15rem] border-black'
                                        src={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZR1UEHgChheu3fJHfr7Ddo1ufyy24S2FC_sAkKtlpMgse2RlV-3hLwzvjdFp1B_y0xhk&usqp=CAU"}
                                    />
                                    
                                </div>
                            </li>
                            <li>
                                <div className='relative w-[16rem] h-[17rem] bg-blue-800 shadow-2xl shadow-black mx-5 rounded-3xl hover:opacity-80'>

                                    <img
                                        className='absolute left-4 top-4 border-4 w-[14rem] h-[15rem] border-black'
                                        src={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZR1UEHgChheu3fJHfr7Ddo1ufyy24S2FC_sAkKtlpMgse2RlV-3hLwzvjdFp1B_y0xhk&usqp=CAU"}
                                    />
                                    
                                </div>
                            </li>

                            <div className='bg-white w-[74rem] h-[1rem] z-10 my-5'/>

                            <li>
                                <div className='relative w-[16rem] h-[17rem] bg-blue-800 shadow-2xl shadow-black mx-5 rounded-3xl hover:opacity-80'>

                                    <img
                                        className='absolute left-4 top-4 border-4 w-[14rem] h-[15rem] border-black'
                                        src={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZR1UEHgChheu3fJHfr7Ddo1ufyy24S2FC_sAkKtlpMgse2RlV-3hLwzvjdFp1B_y0xhk&usqp=CAU"}
                                    />
                                    
                                </div>
                            </li>
                            <li>
                                <div className='relative w-[16rem] h-[17rem] bg-blue-800 shadow-2xl shadow-black mx-5 rounded-3xl hover:opacity-80'>

                                    <img
                                        className='absolute left-4 top-4 border-4 w-[14rem] h-[15rem] border-black'
                                        src={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZR1UEHgChheu3fJHfr7Ddo1ufyy24S2FC_sAkKtlpMgse2RlV-3hLwzvjdFp1B_y0xhk&usqp=CAU"}
                                    />
                                    
                                </div>
                            </li>
                            <li>
                                <div className='relative w-[16rem] h-[17rem] bg-blue-800 shadow-2xl shadow-black mx-5 rounded-3xl hover:opacity-80'>

                                    <img
                                        className='absolute left-4 top-4 border-4 w-[14rem] h-[15rem] border-black'
                                        src={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZR1UEHgChheu3fJHfr7Ddo1ufyy24S2FC_sAkKtlpMgse2RlV-3hLwzvjdFp1B_y0xhk&usqp=CAU"}
                                    />
                                    
                                </div>
                            </li>
                            <li>
                                <div className='relative w-[16rem] h-[17rem] bg-blue-800 shadow-2xl shadow-black mx-5 rounded-3xl hover:opacity-80'>

                                    <img
                                        className='absolute left-4 top-4 border-4 w-[14rem] h-[15rem] border-black'
                                        src={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZR1UEHgChheu3fJHfr7Ddo1ufyy24S2FC_sAkKtlpMgse2RlV-3hLwzvjdFp1B_y0xhk&usqp=CAU"}
                                    />
                                    
                                </div>
                            </li>

                            <div className='bg-white w-[74rem] h-[1rem] z-10 my-5'/>


                            <li>
                                <div className='relative w-[16rem] h-[17rem] bg-blue-800 shadow-2xl shadow-black mx-5 rounded-3xl hover:opacity-80'>

                                    <img
                                        className='absolute left-4 top-4 border-4 w-[14rem] h-[15rem] border-black'
                                        src={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZR1UEHgChheu3fJHfr7Ddo1ufyy24S2FC_sAkKtlpMgse2RlV-3hLwzvjdFp1B_y0xhk&usqp=CAU"}
                                    />
                                    
                                </div>
                            </li>
                            <li>
                                <div className='relative w-[16rem] h-[17rem] bg-blue-800 shadow-2xl shadow-black mx-5 rounded-3xl hover:opacity-80'>

                                    <img
                                        className='absolute left-4 top-4 border-4 w-[14rem] h-[15rem] border-black'
                                        src={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZR1UEHgChheu3fJHfr7Ddo1ufyy24S2FC_sAkKtlpMgse2RlV-3hLwzvjdFp1B_y0xhk&usqp=CAU"}
                                    />
                                    
                                </div>
                            </li>
                            <li>
                                <div className='relative w-[16rem] h-[17rem] bg-blue-800 shadow-2xl shadow-black mx-5 rounded-3xl hover:opacity-80'>

                                    <img
                                        className='absolute left-4 top-4 border-4 w-[14rem] h-[15rem] border-black'
                                        src={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZR1UEHgChheu3fJHfr7Ddo1ufyy24S2FC_sAkKtlpMgse2RlV-3hLwzvjdFp1B_y0xhk&usqp=CAU"}
                                    />
                                    
                                </div>
                            </li>
                            <li>
                                <div className='relative w-[16rem] h-[17rem] bg-blue-800 shadow-2xl shadow-black mx-5 rounded-3xl hover:opacity-80'>

                                    <img
                                        className='absolute left-4 top-4 border-4 w-[14rem] h-[15rem] border-black'
                                        src={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZR1UEHgChheu3fJHfr7Ddo1ufyy24S2FC_sAkKtlpMgse2RlV-3hLwzvjdFp1B_y0xhk&usqp=CAU"}
                                    />
                                    
                                </div>
                            </li>
                            
                        </ul>
                    
                    </div>
                </div>       
            </div>
        </div>

    </div>
  )
}

export default TonMatchingPage